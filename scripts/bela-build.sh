...
# Which host to build. Two effects share this board and this script; the
# default is the one that has run on it.
...
Cross-compiles a Bela host binary for a Bela Gem Stereo, in release mode.
...
Optional:
  BELA_PACKAGE   Which host to build: oxtt-bela (default) or hyperglare-bela.
...
target/aarch64-unknown-linux-gnu/release/$BELA_PACKAGE; scripts/bela-deploy.sh
...
# relays them, and each host's own build.rs turns them into link arguments.
