...
- `crates/hyperglare-render/src/` — the offline renderer that makes M0's comparisons possible: a fixed chord in MIDI note numbers, and EBU R128 matching so an A/B is a comparison of sound rather than of loudness
- `crates/hyperglare-bela/src/` — the board host: `app.rs` is the callback half and touches no libbela, so it compiles and lints off-device; `lib.rs` holds the settings the board is asked for and the `cfg(bela_device)` run. It reports `active_resonators` beside the CPU figure, because the per-sample cost is one filter per resonator that is sounding ([`hyperglare/bela/cpu.md`](hyperglare/bela/cpu.md), [`hyperglare/bela/audio-verification.md`](hyperglare/bela/audio-verification.md))
