...
- `crates/hyperglare-dsp/src/` — the second effect's DSP core: `grid.rs` holds the three resonator geometries and the arithmetic that decides between them, `bands.rs` the map from a frequency to the band it draws its excitation from and the split that produces those bands, `bank.rs` the tuned band-pass bank, its Q cap and the gain law that keeps the geometries and the decay from also being volume controls, `exciter.rs` the two paths that feed it and the per-band gates that are the only thing holding silence-in/silence-out up, `note.rs` the one place that knows what A4 is, `processor.rs` the whole chain and the two placements of the post-drive that M0 has to choose between ([`hyperglare/contracts.md`](hyperglare/contracts.md))
- `crates/hyperglare-args/src/lib.rs` — the flags the renderer and the Bela host share, so a setting cannot mean one thing on a render and another on the board
- `crates/hyperglare-render/src/` — the offline renderer that makes M0's comparisons possible: a fixed chord in MIDI note numbers, and EBU R128 matching so an A/B is a comparison of sound rather than of loudness. Keys are the Bela host's (`--midi-port`); a render's chord does not change
- `crates/hyperglare-bela/src/` — the board host: `app.rs` is the callback half and touches no libbela, so it compiles and lints off-device; `lib.rs` holds the settings the board is asked for and the `cfg(bela_device)` run. It reports `active_resonators` beside the CPU figure, because the per-sample cost is one filter per resonator slot the bank runs — a voice reserves its block whether or not a key is down, so `held_voices` is what says how much of the chord is down ([`hyperglare/bela/cpu.md`](hyperglare/bela/cpu.md), [`hyperglare/bela/audio-verification.md`](hyperglare/bela/audio-verification.md))
