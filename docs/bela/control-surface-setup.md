...
The constants above are not free choices. `crates/effectkit-controls/src/gem.rs` holds
...
`crates/effectkit-controls/src/gem.rs` scales by that ratio, so the top of the travel means
