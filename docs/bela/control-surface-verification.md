...
property tests in `crates/oxtt/src/control/assign.rs` and `crates/effectkit-controls/src/gem.rs`,
...
- **The channel index is the pot.** Sweeping in `A0`→`A5` order moved the
  ...
  `crates/effectkit-controls/src/gem.rs` maps onto `Pots`' fields.
...
`crates/effectkit-controls/src/gem.rs` therefore declares **3.0 counts** for this board,
...
- **Pulling an input mid-session has not been performed**, for the same reason
  ...
  `crates/effectkit-controls/src/gem.rs` already covers the shape of it without hardware:
