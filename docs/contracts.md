...
| `global.low_crossover_hz` | finite, `[40, 2000]` Hz | `CrossoverFreqLow::try_new` | type-specific `nutype` validation error |
| `global.high_crossover_hz` | finite, `[400, 16000]` Hz | `CrossoverFreqHigh::try_new` | type-specific `nutype` validation error |
| `high_crossover_hz >= 2 * low_crossover_hz` | at least one octave apart | `OttParams::validate` | `ConfigError::CrossoverOctave` |
| `band[i].lower_threshold_db`, `upper_threshold_db` | finite, `[-80, 0]` dB | `Threshold::try_new` | type-specific `nutype` validation error |
