...
- Stereo image is preserved under transients that hit only one channel: driving one channel hard does not desync the gain applied to the other channel, since `DualThresholdCompressor::process` takes a single detector power `p` and returns a single `dynamic_gain` applied to both `wet_left` and `wet_right` (`crates/oxtt-dsp/src/dsp.rs`, `crates/oxtt-dsp/src/dsp/compressor.rs`).
...
- Verified by `detector_power_is_symmetric_peak_linked` (`crates/oxtt-dsp/src/dsp/envelope.rs`) and `band_applies_one_dynamic_gain_to_asymmetric_stereo_input` (`crates/oxtt-dsp/src/dsp.rs`); startup/reset behavior is covered by `init_state_yields_0db_gain` (`crates/oxtt-dsp/src/dsp/compressor.rs`), `init_and_reset_snap_to_threshold_powers`, and `reset_after_use_snaps_back_to_threshold_powers` (`crates/oxtt-dsp/src/dsp/envelope.rs`).
