...
- The unprocessed 3-band sum (`low + mid + high`) is flat within +/-0.1 dB from 20 Hz to `0.45 * sample_rate`, independent of `depth`, because `A_low(z) * A_high(z)` is a unity-gain all-pass response (verified by `reconstruction_is_flat_*` and `phase_compensator_alone_is_unity_gain` in `crates/oxtt-dsp/src/dsp/crossover.rs`).
- `depth = 0` is well-defined and testable in isolation: it must equal "raw LR4 reconstruction -> per-band input gain -> sum -> output gain" exactly, not raw passthrough (verified by `depth_zero_matches_pure_crossover_reconstruction` in `crates/oxtt-dsp/src/dsp.rs`). A static input gain is linearity-equivalent to applying it before the split, but a moving gain deliberately does not alter the crossover's filter history.
- The low branch pays the extra CPU cost of one more `Lr4` pair (two biquad cascades) per channel, and its filter state must never be shared with the mid/high split's own `Lr4` pair — sharing would corrupt both computations while the smoothed cutoff is mid-transition. This drives the `ChannelSplitter` struct layout in `crates/oxtt-dsp/src/dsp/crossover.rs`, which keeps `low_split_*`, `high_split_*`, and `phase_comp_*` as six separate `Lr4` instances.
