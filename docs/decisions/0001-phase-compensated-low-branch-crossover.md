...
- `depth = 0` is well-defined and testable in isolation: it must equal "input gain -> LR4 reconstruction -> output gain" exactly, not "input gain -> raw passthrough" (verified by `depth_zero_matches_pure_crossover_reconstruction` in `src/dsp.rs`).
