...
- `depth = 0` is well-defined and testable in isolation: it must equal "raw LR4 reconstruction -> per-band input gain -> sum -> output gain" exactly, not raw passthrough (verified by `depth_zero_matches_pure_crossover_reconstruction` in `src/dsp.rs`). A static input gain is linearity-equivalent to applying it before the split, but a moving gain deliberately does not alter the crossover's filter history.
