...
//! *immediate* dependent's build script, so `hyperglare-bela` cannot inherit them
...
//! crate-wide prohibitions from `clippy.toml` (docs/hyperglare/contracts.md §6) do not
//! describe it.
#![allow(
    clippy::disallowed_macros,
    reason = "a build script talks to Cargo over stdout; docs/hyperglare/contracts.md §6 governs the audio callback, not build time"
)]
...
/// be repeated here for `#[cfg(bela_device)]` to mean anything in hyperglare's own
